...
public class DirectExchangeConsumerService {
	...
	public void consumeAlgoThree(String message) {
		...
		if(!message.toLowerCase().contains("ram")) {
			...
			//success
			logger.info("consumeAlgoThree: succesfully processed message {} with algo three",message);
		}
	}
	...
	@RabbitListener(queues = {"dead-letter-queue-three"})
	public void consumeDeadLetterAlgoThree(String message) {
		logger.info("consumeDeadLetterAlgoThree: Sending email to admin for message",message);
	}
}
